                        ...
                        <v-card :disabled="isLoginDisable" v-if="!isLoginDisable">
                          ...
                          <v-card-text :class="checkMobile?'pa-0':'pb-2'">
                            <v-tabs v-model="userCd" fixed-tabs color="primary">
                              ...
                              <v-tab value="JNT" v-if="businessInfo.value === 'ADV' || businessInfo.value === 'LAW'">복수가입 회원</v-tab>
                              <v-tab value="JNT" v-if="businessInfo.value === 'PAT'">합동가입 회원</v-tab>
                              <v-tab value="COR" v-if="businessInfo.value === 'PAT'">법인가입 회원</v-tab>
                            ...
                            <v-window v-model="userCd">
                                ...
                                <div class="my-4 login-subtext">
                                    ...
                                    <p v-if="businessInfo.value === 'PAT'">법인 회원은 하나의 아이디만 부여되며 본점, 지점별로 중복가입 되지 않습니다.                                    <v-icon class="ml-1" size="small">mdi-alert-circle-outline</v-icon>
                                      <v-tooltip activator="parent" location="top">
                                        법인가입 안내<v-icon class="ml-1" size="small">mdi-alert-circle-outline</v-icon>
                                        <v-divider class="my-1"/>
                                        법인 가입 시, 보험의 피보험자는 법인이 되며, 법인의 책임이 보장됩니다.
                                      </v-tooltip>
                                    </p>
                                </div>
                              ...
                              <v-window-item value="two">
                                <div class="my-4 login-subtext">
                                  <p v-if="businessInfo.value === 'CAA'">법인/합동가입 회원은 하나의 아이디만 부여되며 본점, 지점별로 중복가입 되지 않습니다.</p>
                                </div>
                                <VTextFieldWithValidation
                                    name="COR_user_id"
                                    placeholder="사업자번호"
                              ...
                              <v-window-item value="JNT">
                                <div class="my-4 login-subtext">
                                  ...
                                  <p v-if="businessInfo.value === 'LAW'">복수가입(보상한도 공유가입)회원은 사무소 또는 법인명으로 하나의 아이디만 부여되며 본점, 지점별로 중복가입 되지 않습니다.
                                    <v-icon class="ml-1" size="small">mdi-alert-circle-outline</v-icon>
                                    <v-tooltip activator="parent" location="top">
                                      복수가입 안내<v-icon class="ml-1" size="small">mdi-alert-circle-outline</v-icon>
                                      <v-divider class="my-1"/>
                                      복수가입 시, 보험의 피보험자는 보상한도를 공유하여 가입하는 각 법무사가 되며, <br/>사무소 또는 법인이 피보험자가 되지 않습니다.
                                    </v-tooltip>
                                  </p>
                                  <p v-if="businessInfo.value === 'PAT'">합동가입(보상한도 공유가입)회원은 사무소 또는 법인명으로 하나의 아이디만 부여되며 본점, 지점별로 중복가입 되지 않습니다.
                                    <v-icon class="ml-1" size="small">mdi-alert-circle-outline</v-icon>
                                    <v-tooltip activator="parent" location="top">
                                      합동가입 안내<v-icon class="ml-1" size="small">mdi-alert-circle-outline</v-icon>
                                      <v-divider class="my-1"/>
                                      합동가입 시, 보험의 피보험자는 보상한도를 공유하여 가입하는 각 변리사가 됩니다. <br/>사무소 또는 법인이 피보험자가 되지 않으며, 가입한 변리사의 책임만 보장됩니다.
                                    </v-tooltip>
                                  </p>
