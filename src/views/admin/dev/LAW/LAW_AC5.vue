            ...
            <v-table density="compact" fixed-header height="220px">
              ...
              <tbody v-if="InsuranceList.length" class="">
                <tr v-for="(row, index) in InsuranceList" :key="index" @click="selectedRow = row" @click.prevent="onPageView(row)" :class="{ selected: selectedRow === row, 'cursor-pointer': true }">
                  ...
                  <td>{{  (Number(row?.insr_tot_amt) - Number(row?.insr_relief)).toLocaleString() }}</td>
      ...
      <v-card>
        <v-card-title class="d-flex flex-wrap px-0 pt align-center">
          ...
          <div class="ml-auto">
            ...
            <v-btn variant="outlined" size="small" class="mr-1" @click="fnAdd()">신규 입금</v-btn>
...
import {
  DOWNLOAD_EXCEL,
  UPLOAD_EXCEL_INSURANCE_LAW_TRE_IND,
  UPLOAD_EXCEL_INSURANCE_LAW_TRE_JNT
} from '@/util/excelupdn';
...
async function fnSave() {
  ...
  if (isRun) {
    const resultData = await apiADMIN.setLAW_TRX([insuranceDTO.value]);
...
async function fnSearch() {
  InsuranceList.value = [];
  const resultData = await apiADMIN.getLAW_TRX(searchParams.value.data);
...
async function fnExcelDownload() {
  ...
  try {
    if (isRun) {
      let resultData = await apiADMIN.getLAWExcel(searchParams.value.data);
