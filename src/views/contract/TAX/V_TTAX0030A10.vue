  ...
  <v-row v-if="!isNoData">
    <v-col cols="12" sm="12" class="mt-16">
      <div class="d-flex justify-space-between align-end">
        ...
        <div>
          <v-btn variant="flat" @click="onPageMove('insert')"  v-if="(renewalInsrYear - maxInsrYear > 1) || (newInsrYN=='Y' && renewalInsrUUID == null && InsuranceList.filter(item => (item.status_cd === '80'||item.status_cd === '90')&&item.insr_year == renewalInsrYear ).length == 0)">신규 가입</v-btn>&nbsp;
      ...
      <v-table class="v-board-table size-large mt-4">
        ...
        <tr v-for="(row, index) in InsuranceList">
          ...
          <template v-else>
            <td colspan="5" class="text-center">{{ row.user_nm }} [{{(row.user_cd == 'IND')?'개인': '법인'}}] 가입</td>
          </template>
        </tr>
        </tbody>
        <tbody v-else>
