<template>
  ...
  <v-row justify="center" v-if="isNoData" >
    <v-col cols="12" sm="6">
      ...
      <div class="d-flex justify-center">
        <v-btn :size="checkMobile.isMobile?'default':'x-large'" variant="flat" color="primary" to="/contract/PAT/V_TPAT0030A11" depressed class="px-10">가입신청</v-btn>
      </div>
    </v-col>
  </v-row>
      ...
      <v-table class="v-board-table size-large mt-4">
        ...
        <tbody v-if="InsuranceList.length">
          <tr v-for="(row, index) in InsuranceList">
            ...
            <template v-else>
              <td colspan="5" class="text-center">{{ row.user_nm }} [{{(row.user_cd == 'IND')?'개인': (row.user_cd == 'JNT')?'복수':'법인'}}] 가입</td>
            </template>
  ...
  <!-- 가입신청서 시작 -->
  <V_TPAT0030P20 :insurance_uuid="insuranceUUID" :isPdf=true v-if="isInsuranceFormDialog" @close="onInsuranceFormClose" />
  <!-- 가입증명서 종료 -->

  <!-- 가입증명서 시작 -->
  <V_TPAT0030P30 :insurance_uuid="insuranceUUID" :isPdf=true v-if="isCertificatePrintFramDialog" @close="onCertificatePrintFrameClose" />
  <!-- 가입증명서 종료 -->
...
<script setup lang="ts">
  ...
  const onPageMove = (actionType:string) => {
    if(actionType == 'renewal') {
      router.push({ path: '/contract/PAT/V_TPAT0030A11/' + renewalInsrUUID.value , query :{renewal : 'Y'} });
    }else  if(actionType == 'insert') {
      router.push({ path: '/contract/PAT/V_TPAT0030A11'});
    }
  };
      ...
      messageBoxDTO.value.setConfirm('확인', '신청이력이 있습니다. 수정하시겠습니까?', insurance_uuid, (result, params) => {
        if (result) {
          if(insr_year === renewalInsrYear.value) {
            router.push({ path:'/contract/PAT/V_TPAT0030A11/' + params, query :{renewalUpdate : 'Y'} });
          }else {
            router.push('/contract/PAT/V_TPAT0030A11/' + params);
          }
        }
      });
    
    }else {
      path = '/contract/PAT/V_TPAT0030A12/' + insurance_uuid;
      router.push(path);
    }
  ...
  onMounted(async () => {    
      ...
      const resultData = await apiPAT0030a.getDBSelList(params);
