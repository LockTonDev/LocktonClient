        ...
        <v-window v-model="tab">
          ...
          <v-window-item value="2" :disabled="isReadOnlyAll">
            <v-row class="v-box-table py-10">
                  ...
                  <v-col cols="12" sm="12" class="v-col">
                    ...
                    <div class="data-col">
                      <v-btn-toggle
                          ...
                          :disabled="isReadOnlyAll"
                      >
                        <v-btn color="primary" class="flex-grow-0 w-30" value="N"
                        >사용안함</v-btn
                        >
                        <v-btn color="primary" class="flex-grow-0 w-30" value="Y"
                        >사용함</v-btn
                        >
                      </v-btn-toggle>
                      <p><i class="mdi mdi-alert-circle-outline mr-2"></i>기일관리프로그램 사용 시 보험료 10% 할인적용</p>
                  ...
                  <v-col cols="12" sm="12" class="v-col" v-if="insuranceDTO.user_cd=='COR'">
                    ...
                    <div class="data-col w-100">
                      <div class="custom-file-upload">
                        <input type="file" @change="uploadFile"  id="file" class="file-input" />
              ...
              <!-- 변리사 명단 시작 -->
              <v-col cols="12" sm="12" class="py-0 px-14 mb-10" v-if="insuranceDTO.user_cd === 'JNT'">
            ...
            <v-row class="v-box-table">
              <v-col cols="12" class="px-16 py-10">
                ...
                >
                  ...
                  <li>
                    보험회사 : 흥국화재해상보험㈜ <span class="text-caption mx-3">|</span>보험중개사 : 록톤컴퍼니즈코리아손해보험중개(주)
                  </li>
                  <li>
                    보험료 입금 계좌번호 :
              ...
              <v-col cols="12" class="px-14 py-10 border-top-lightgray-1">
                ...
                <div class="text-right">
                  ...
                  <p class="text-body-2 mt-2">
                    ...
                    <span class="d-inline-block w-sm-min-110 px-4 py-1" v-else-if="insuranceDTO.cbr_data.length>0">
                      {{ insuranceDTO.cbr_data[0].cbr_nm }} 외 {{insuranceDTO.cbr_cnt - 1}}명
                    </span>
                  </p>
                </div>
...
import TermsOfPolicy from './V_TPAT0030P01.vue';      // PDF 다운로드
 ...
 */
 const getInsrAmt = (
  ...
  let sKey = sKey1 + '|' + sKey2 + '|' + sKey3.getValueBySplit(0);
...
async function chkSaleRtJNT(list: any, rowIdx: number) {
  ...
  const result = await apiPAT0030a.getSaleRtNDupInfo(params);
  if (result.success) {
...
function uploadFile(event) {
  ...
  insuranceDTO.value.insr_income_filename = event.target.files[0].name;
}

function isNotPsnlBrdn(value: any) {
  const start_key = clm_lt_amt.value + '|' + value
...
onMounted(async () => {
  ...
  if(renewalYN.value === 'Y' && insuranceUUID.value !== '') {
    ...
    const renewalData = await apiPAT0030a.getDBSel(params);
...
function changeTakeAmount() {
  ...
  let num_insr_take_amt = insr_take_amt.value.replace(/[^0-9]/g, "");
  if (num_insr_take_amt.startsWith('0') && num_insr_take_amt.length > 1){
    num_insr_take_amt =  num_insr_take_amt.replace(/^0+/, '');
  }
  insuranceDTO.value.insr_take_amt = num_insr_take_amt

  // 천의 자리마다 쉼표(,) 추가
  num_insr_take_amt = num_insr_take_amt.replace(/\B(?=(\d{3})+(?!\d))/g, ",");

  // 사용자 입력 필드에 쉼표(,)가 추가된 값 설정
  insr_take_amt.value = num_insr_take_amt;
}
...
function resetCBRData() {
  messageBoxDTO.value.setConfirm('변리사 명단', '변리사 명단을 초기 조회상태로 초기화 하시겠습니까?', null, (result, params) => {
    if (result) {
