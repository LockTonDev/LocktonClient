  ...
  <v-row>
    <v-col cols="12" sm="12" class="py-16">
      <div class="d-flex justify-space-between align-end">
        <p class="text-body-1">
          전체 <span class="color-primary font-weight-bold">{{ d_TCAA0040AList.length }}</span
          >건
        </p>
    ...
    <v-card>
      ...
      <v-card-text class="pa-6">
        <v-row class="v-board-table ma-0">
          ...
          <v-col cols="12">
            <div class="head-col">내용<sup class="text-error">*</sup></div>
            <div class="data-col w-100">
              <p class="w-100 text-gray">* 해지사유 : 폐업, 휴업 등</p>
      ...
      <v-card-text class="pa-6">
        <v-row class="v-board-table ma-0">
          ...
          <!-- <v-col cols="12">
            ...
            <div class="data-col">
              <pre class="pre">{{ d_TCAA0040A.proc_content }}</pre>
            </div>
          </v-col> -->
        </v-row>
      </v-card-text>
...
async function onRegDialog() {
  ...
  d_TCAA0040A.value.apply_cd = 'ACCIND_30';
  d_TCAA0040A.value.apply_posted_dt = today.value;
  d_TCAA0040A.value.apply_dt = today.value;
...
async function onSubmit() {
  ...
  const retData = await apiA_TTAX0040A.setDBIns(d_TCAA0040A.value);
  console.log(retData);
  if (retData.success) {
    getItems();
...
onMounted(async () => {
  ...
  applyItems.value = items.filter(item => item.value.startsWith('ACCIND'));
