       ...
       <v-card-text class="pa-6">
        <v-row class="v-board-table ma-0">
          ...
          <!-- <v-col cols="12">
            ...
            <div class="data-col"><pre class="pre">{{ d_TLAW0040A.proc_content  }}</pre></div>
          </v-col> -->
...
<script setup lang="ts">
  ...
  const d_TLAW0040A = ref(new D_T0040ADTO());
  const d_TLAW0040AList = ref<D_T0040ADTO[]>([]);
 ...
 /**
   ...
   */
   async function onRegDialog() {
    ...
    d_TLAW0040A.value.clear();
    d_TLAW0040A.value.apply_posted_dt = today;
    d_TLAW0040A.value.apply_dt = today;
    d_TLAW0040A.value.proc_cd = '10'; //접수
    d_TLAW0040A.value.user_cd = _AUTH_USER.value.userCd;
    ...
    if(data.length > 0) {
      d_TLAW0040A.value.insurance_no = data[0].insurance_no;
      d_TLAW0040A.value.insurance_uuid = data[0].insurance_uuid;
      d_TLAW0040A.value.insurance_user_nm = data[0].user_nm;
...
async function checkValidation() {
  ...
  const isValidate = await validationSchema.validate(d_TLAW0040A.value, { abortEarly: false })
    .then(() => {
      return true;
    })
    .catch(error => {
...
async function onSubmit() {
  ...
  const retData = await apiA_TLAW0040A.setDBIns(d_TLAW0040A.value);
  if(retData.success) {
    getItems();
  ...
  async function onViewDtl(apply_no:number) {
    ...
    const retData = await apiA_TLAW0040A.getDBSel(params);
    d_TLAW0040A.value = new D_T0040ADTO(retData.data);
  ...
  const getItems = async () => {
    ...
    const { data } = await apiA_TLAW0040A.getDBSelList(params);

    d_TLAW0040AList.value = data.items;
