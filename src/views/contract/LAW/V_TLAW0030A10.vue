<template>
          ...
          <tr >
            ...
            <th class="font-weight-medium text-center text-body-1 text-no-wrap">
              ...
              <v-tooltip activator="parent" location="top">
                ...
                개시일인 8월 2일 이후 영업일 기준 3일 후부터 출력 가능합니다.<br/>
  ...
  <!-- 가입신청서 시작 -->
  <V_TLAW0030P20 :insurance_uuid="insuranceUUID" :isPdf=true v-if="isInsuranceFormDialog" @close="onInsuranceFormClose" />
  <!-- 가입증명서 종료 -->

  <!-- 가입증명서 시작 -->
  <V_TLAW0030P30 :insurance_uuid="insuranceUUID" :isPdf=true v-if="isCertificatePrintFramDialog" @close="onCertificatePrintFrameClose" />
  <!-- 가입증명서 종료 -->
...
<script setup lang="ts">
  ...
  const onPageMove = (actionType:string) => {
    if(actionType == 'renewal') {
      router.push({ path: '/contract/LAW/V_TLAW0030A11/' + renewalInsrUUID.value , query :{renewal : 'Y'} });
    }else  if(actionType == 'insert') {
      router.push({ path: '/contract/LAW/V_TLAW0030A11'});
    }
  };
      ...
      messageBoxDTO.value.setConfirm('확인', '신청이력이 있습니다. 수정하시겠습니까?', insurance_uuid, (result, params) => {
        if (result) {
          if(insr_year === renewalInsrYear.value) {
            router.push({ path:'/contract/LAW/V_TLAW0030A11/' + params, query :{renewalUpdate : 'Y'} });
          }else {
            router.push('/contract/LAW/V_TLAW0030A11/' + params);
          }
        }
      });
    }else {
      path = '/contract/LAW/V_TLAW0030A12/' + insurance_uuid;
      router.push(path);
    }
  };
  ...
  onMounted(async () => {
    ...
    if(InsuranceList.value.length > 0 && InsuranceList.value[0]?.cbr_data?.length>0) {
