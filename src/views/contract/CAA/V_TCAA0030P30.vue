<template>
  ...
  >
      ...
      <v-card-title class="d-flex align-center justify-space-between">
          ...
          <div class="d-flex justify-space-between align-end">
              <v-btn :size="checkMobile.isMobile?'x-small':'default'" variant="flat" color="dark" @click="onExportPDF('down')">저장</v-btn>&nbsp;
  ...
  <div class="d-none">
      <div id="printDiv" class="certificatePrintFrame-wrap">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td colspan="2" rowspan="3">{{(Number(insuranceDTO.insr_amt) + Number(insuranceDTO.spct_data?.insr_amt? insuranceDTO.spct_data?.insr_amt : 0 )).toLocaleString()}}원</td>
                        </tr>
                        <tr>
                            <th colspan="2">부정행위 보상한도(1청구당/연간총)</th>
                            <th>자기부담금(1청구당)</th>
                        </tr>
                        <tr>
        ...
        <div class="print-wrap v2 page1" v-if="insuranceDTO.cons_join_yn == 'Y'">
          <header>
            <div class="text-right">
              <img src="../../../assets/images/img-logo-meritz1.png" alt="" style="height: 20px; margin-bottom: 10px;"/>
            </div>
          ...
          <main>
            ...
            <table class="table">
              ...
              <tr>
                <td colspan="2">{{insuranceDTO.cons_data.insr_clm_lt_amt.getValueBySplit(1)}} / {{insuranceDTO.cons_data.insr_year_clm_lt_amt}}</td>
                <td>{{insuranceDTO.cons_data.insr_psnl_brdn_amt.getValueBySplit(1)}}</td>
                <td colspan="2" rowspan="3">{{Number(insuranceDTO.cons_data.insr_amt).toLocaleString()}}원</td>
              </tr>
              </tbody>
            </table>
            <h2 class="title-wrap">
...
onMounted(async () => {
  ...
  if(insuranceDTO.value.cons_join_yn != null && insuranceDTO.value.cons_join_yn == 'Y' ){
    insuranceDTO.value.cons_data.cbr_data = exList(insuranceDTO.value.cons_data.cbr_data)
...
const exList = (list: any) => {
  if(list) {
    return list.filter(item => item.status_cd == '80')
  }else{
    return list
  }
}
